public function updatePickup(Request $request, $booking_id)
{
    ...
    try {
        ...
        if ($saleRow) {
            ...
            // (A) Optional CDW line as its own log
            if ($cdwlog && (float)$cdwlog->amount > 0) {
                $w = $weekOf($startAt);
                $m = $monthCol($startAt);
                $cdwAmt = (float) $cdwlog->amount;

                $payload = [
                    'sale_id'    => $saleId,
                    'daily_sale' => $cdwAmt,
                    'type'       => 'cdw',
                    'year'       => (int) $startAt->year,
                    'date'       => $startAt->format('Y-m-d H:i:s'),
                    'created'    => now(),
                ];
                $payload[$w] = $cdwAmt;
                if ($m) $payload[$m] = $cdwAmt;

                SaleLog::create($payload);
            }

            // (B) Break down rental sale into daily/hour logs proportionally
            $hoursTotal = max(0, $startAt->diffInHours($endAt));
            $daysFull   = intdiv($hoursTotal, 24);
            $hoursLeft  = $hoursTotal % 24;
            $remaining  = (float) $saleAmount;

            // (B0) Preserve legacy "firstday" marker for rentals spanning >= 1 day
            if ($daysFull >= 1) {
                $dt = (clone $startAt);
                $w  = $weekOf($dt);
                $m  = $monthCol($dt);
            ...
            if ($daysFull === 0) {
                // < 1 day: single 'hour' row for full amount
                $dt  = (clone $startAt);
                $w   = $weekOf($dt);
                $m   = $monthCol($dt);
                ...
                // >= 1 day: distribute evenly across full days + leftover hours
                $totalUnits = $daysFull + ($hoursLeft > 0 ? ($hoursLeft / 24) : 0);
                $perDay     = $totalUnits > 0 ? round($saleAmount / $totalUnits, 2) : 0.00;

                $dt = (clone $startAt);
                for ($i = 1; $i <= $daysFull; $i++) {
                    // Use per-day; adjust on last full day if needed
                    $amt = ($i < $daysFull || $hoursLeft > 0)
                        ? min($perDay, $remaining)
                        : round($remaining, 2);
                ...
                if ($hoursLeft > 0 && $remaining > 0.00) {
                    $w = $weekOf($dt);
                    $m = $monthCol($dt);
                    $amt = round(min($remaining, $perDay * ($hoursLeft / 24)), 2);
                ...
                // Correction for rounding residue, attach to last full-day date
                if ($remaining > 0.00) {
                    $w   = $weekOf($dt);
                    $m   = $monthCol($dt);
                    $amt = round($remaining, 2);
