...
@section('page-action')
    <div>
        <a href="{{ route('reservation.index') }}"  data-bs-toggle="tooltip" data-bs-original-title="{{__('All reservation')}}" class="btn btn-sm btn-primary btn-icon">
        ...
        <div class="col-sm-12">
            <div class="row">
                @foreach ($statues as $key => $status)
                    @php
                        $reservations = App\Models\reservation::where('workspace', getActiveWorkSpace())
                            ->where('status', $key)
                            ->get();
                        $countstatus = $reservations->count();

                        $total = $countstatus != 0 ? number_format($countstatus / $total_reservations, 4) : 0;
