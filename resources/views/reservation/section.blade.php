        ...
        var $repeater = $(selector + ' .repeater').repeater({
            ...
            hide: function(deleteElement) {
                if (confirm('Are you sure you want to delete this element?')) {
                    ...
                    {
                        $.ajax({
                            url: '{{ route('reservation.product.destroy') }}',
                            type: 'POST',
                            headers: {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var product_select = `<select class="form-control product_id item js-searchBox" name="${itemSelect}"
                                        placeholder="Select Item" data-url="{{route('reservation.product')}}" required = 'required'>
                                        </select>`;
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.ajax({
                            ...
                            cache: false,
                            success: function(data) {
                                var reservationItems = JSON.parse(data);

                                if (reservationItems != null) {
                                    var amount = (reservationItems.price * reservationItems.quantity);

                                    $(el.parent().parent().find('.quantity')).val(reservationItems
                                        .quantity);
                                    $(el.parent().parent().find('.price')).val(reservationItems.price);
                                    $(el.parent().parent().find('.discount')).val(reservationItems
                                        .discount);
                                } else {
...
@if ($type == 'product')
    ...
    <div class="card repeater" @if ($acction == 'edit') data-value='{!! json_encode($reservation->items) !!}' @endif>
                    ...
                    <tbody class="ui-sortable" data-repeater-item>
                        <tr>
                            ...
                            <td width="25%" class="form-group pt-0 product_div">
                                    <select name="item" class="form-control product_id item  js-searchBox" data-url="{{route('reservation.product')}}" required>
