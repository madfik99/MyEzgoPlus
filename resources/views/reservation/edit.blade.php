...
@push('scripts')
    ...
    <script>
        $(document).on('change', '.product_id', function() {
            var in_type = $('#reservation_type').val();
            if (in_type == 'product') {
                items($(this), 'Account');
    ...
    <script>
        function SectionGet(type = 'product', project_id = "0", title = 'Project') {
            ...
            $.ajax({
                type: 'post',
                url: "{{ route('reservation.section.type') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    type: type,
    ...
    <script>
        $(document).on('change', "[name='reservation_type_radio']", function() {
            var val = $(this).val();
            $(".reservation_div").empty();
            var reservation_module = '{{ $reservation->reservation_module }}';
            if (val == 'product') {
                ...
                $(".reservation_div").append(label);
                $("#reservation_type").val('product');

                if (reservation_module == 'account') {
                ...
                var label =
                    ` {{ Form::label('project', __('Project'), ['class' => 'form-label']) }}<x-required></x-required> {{ Form::select('project', $projects, $reservation->category_id, ['class' => 'form-control', 'required' => 'required']) }}`
                $(".reservation_div").append(label);
                $("#reservation_type").val('project');
                var project_id = $("#project").val();

                if (reservation_module == 'taskly') {
                    $("#acction_type").val('edit');
                } else {
                    $("#acction_type").val('create');
                }
                ...
                $(".reservation_div").append(label);
                $("#reservation_type").val('parts');

                if (reservation_module == 'cmms') {
                    $("#acction_type").val('edit');
                } else {
                    $("#acction_type").val('create');
                }
    ...
    @if (module_is_active('Account') && $reservation->reservation_module == 'account')
        <script>
    ...
    @elseif (module_is_active('Taskly') && $reservation->reservation_module == 'taskly')
        <script>
            $(document).ready(function() {
                $('.tax_project_div').removeClass('d-none');
    ...
    @elseif (module_is_active('CMMS') && $reservation->reservation_module == 'cmms')
        <script>
            $(document).ready(function() {
                $('.category_id').addClass('d-none');
                $('.project_id').addClass('d-none');
...
@section('content')
    <div class="row">
        {{ Form::model($reservation, ['route' => ['reservation.update', $reservation->id], 'method' => 'PUT', 'class' => 'w-100', 'enctype' => 'multipart/form-data', 'class' => 'needs-validation', 'novalidate']) }}
        @if ($reservation->reservation_module == 'account')
            <input type="hidden" name="reservation_type" id="reservation_type" value="product">
        @elseif ($reservation->reservation_module == 'taskly')
            <input type="hidden" name="reservation_type" id="reservation_type" value="project">
        @elseif ($reservation->reservation_module == 'cmms')
            <input type="hidden" name="reservation_type" id="reservation_type" value="parts">
        @endif
                    ...
                    <div class="row">
                        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                            <div class="row" id="customer-box">
                                <div class="form-group col-md-6 " id="account-box">
                                    ...
                                    <input type="hidden" name="account_type" value="{{ $reservation->account_type ?? '' }}">
                                </div>
                                <div class="form-group col-md-6">
                                    {{ Form::label('customer_id', __('Customer'), ['class' => 'form-label']) }}<x-required></x-required>
                                    {{ Form::select('customer_id', $customers, null, ['class' => 'form-control', 'id' => 'customer', 'data-url' => route('reservation.customer'), 'required' => 'required', 'placeholder' => 'Select Customer']) }}
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select
                                            ...
                                            name="reservation_type_radio" required="" id="billing_type">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="reservation_template" class="form-label">{{ __('Template') }}</label>
                                        <select class="form-control" name="reservation_template" id="reservation_template">
                                            <option value="">{{ __('Select Template') }}</option>
                                            @foreach (templateData()['templates'] as $key => $template)
                                                <option value="{{ $key }}"
                                                    {{ $reservation->reservation_template == $key ? 'selected' : '' }}>
                                                    {{ $template }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        {{ Form::label('reservation_number', __('reservation Number'), ['class' => 'form-label']) }}
                                        <div class="form-icon-user">
                                            <input type="text" class="form-control" value="{{ $reservation_number }}"
                                                readonly>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group reservation_div">
                                        @if (module_is_active('Account'))
                                        ...
                                        @if (module_is_active('Taskly'))
                                            ...
                                            {{ Form::select('project', $projects, $reservation->category_id, ['class' => 'form-control project_id', 'required' => 'required']) }}
                                        @endif
                                        @if (module_is_active('CMMS'))
                                ...
                                @if (module_is_active('Taskly'))
                                    <div
                                        ...
                                        <div class="form-group">
                                            {{ Form::label('tax_project', __('Tax'), ['class' => 'form-label']) }}<x-required></x-required>
                                            {{ Form::select('tax_project[]', $taxs, !empty($reservation->items->first()->tax) ? explode(',', $reservation->items->first()->tax) : null, ['class' => 'form-control get_tax multi-select choices', 'multiple' => 'multiple', 'id' => 'tax_project']) }}
                                        </div>
                                    </div>
                                @endif
                                                ...
                                                <optgroup label="{{ $typeName }}">
                                                    @foreach ($subtypes as $subtypeId => $subtypeData)
                                                        ...
                                                        @foreach ($subtypeData['chart_of_accounts'] as $chartOfAccount)
                                                            <option value="{{ $chartOfAccount['id'] }}" {{ $chartOfAccount['id'] == $reservation->account_id ? 'selected' : ''}}>
                                                                &nbsp;&nbsp;&nbsp;{{ $chartOfAccount['account_name'] }}
                                                            </option>
                                                            @foreach ($subtypeData['subAccounts'] as $subAccount)
                                                                @if ($chartOfAccount['id'] == $subAccount['parent'])
                                                                <option value="{{ $subAccount['id'] }}" class="ms-5" {{ $subAccount['id'] == $reservation->account_id ? 'selected' : ''}}> &nbsp; &nbsp;&nbsp;&nbsp; {{' - '. $subAccount['account_name'] }}</option>
                                                                @endif
                                                            @endforeach
                                                        @endforeach
                                                    @endforeach
                                    ...
                                    <div class="col-md-12">
                                        <div class="tab-pane fade show" id="tab-2" role="tabpanel">
                                            @include('custom-field::formBuilder', [
                                                'fildedata' => !empty($reservation->customField)
        ...
        <div class="modal-footer mb-3">
            <input type="button" value="{{ __('Cancel') }}" onclick="location.href = '{{ route('reservation.index') }}';"
...
@push('scripts')
        ...
        $(document).ready(function() {
            ...
            $('#account_type').on('change', function() {
                ...
                if (optionsMap.hasOwnProperty(selectedOption)) {
                    ...
                    if (value !== null) {
                        $('[name="reservation_type_radio"]').append('<option value="' + value + '" >' +
    ...
    <script>
        $(document).ready(function() {
            var valueToMatch = "{{ $reservation->account_type }}";
